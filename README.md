...
- **最终只读复核** — 达到最大轮次时自动对最后一轮修复执行一次独立复核，避免"修了但没人检查"的收尾盲区
- **中文审查报告** — 结构化中文报告，包含问题清单、修复详情、误报分析、最终复核结果
...
Phase 5: LOOP ────── 判断: 通过? 达到上限? 继续?
    ...
    │ (继续)             │ (结束)
    └──→ Phase 2         ▼
              Phase 6: FINAL VERIFICATION ── 仅当达到最大轮次且
                       │                      有过修复时执行
                       │                      只读复核，不再修改代码
                       ▼
                  Phase 7: REPORT ── 生成中文审查报告（含最终复核结果）
```

> **关于 Phase 6 (最终复核)**：当审查因 PASS 提前退出，或所有问题都被判定为误报时会自动跳过；只有"达到最大轮次且至少应用了一次修复"才会执行。复核为只读模式，不会再修改任何代码——它的作用是给用户一个独立证据，告诉你最后一轮的修复到底有没有解决问题。
...
**复核状态**: SKIPPED
**跳过原因**: first_or_intermediate_cycle_passed

> 第 2 轮 Codex 已直接判定 PASS，无需额外复核。
> 当审查因达到最大轮次而退出时，本节会展示一次只读复核的判定 (PASS / PARTIAL / FAILED)、历史问题状态表，以及复核中发现的任何严重回归。
